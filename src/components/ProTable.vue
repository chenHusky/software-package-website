...
const getCheckedColumnList = () => {
  ...
  try {
    ...
    if (obj instanceof Object && obj[props.id]) {
      return obj[props.id];
    } else {
      return null;
    }
...
const filterTableData = () => {
  ...
  // 异步的则返回过滤条件
  if (props.isAsync) {
    emits('filter', filterParam.value);
  } else {
...
<template>
  <div :style="{ 'max-height': calcContainerHeight, width }" class="pro-table">
    <ElTable
      :id="id"
      :max-height="height"
      :style="{ width: '100%' }"
